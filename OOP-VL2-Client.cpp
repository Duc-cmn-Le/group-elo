...
private: 
        ...
        string command;
        string fname[7] = {"inp1.txt", "inp2.txt", "inp3.txt", "inp4.txt", "inp5.txt", "inp6.txt", "inp7.txt"};
        ofstream fLOG;
        ifstream fCONTEST, fSHOW;
        ifstream fINP[7];
        int Z[7] = {10, 10, 10, 10, 10, 10, 12};

        System() {} // Private constructor to prevent creating instance
        System(const System&) {}
...
public: 
        ...
        static bool ratingsRank(contestant A, contestant B);
        static bool idRank(contestant A, contestant B);
...
int main() {
    System &TriTuePTNK = System:: Singleton();
    TriTuePTNK.process();
    return 0;
}
...
long double System:: expect(long double rating_A, long double rating_B) {
...
int System:: K_const(long double ratings) {
	if (ratings < 1400) {
		return K_1200;
...
bool System:: ratingsRank(contestant A, contestant B) {
	return (A.ratings > B.ratings);
}

bool System:: idRank(contestant A, contestant B) {
	return (A.id < B.id);
}

long double System:: median(vector <contestant> scores, int size, int type, int block, int question) {
	if (size == 1) {
...
void System:: rating_update(int block, int question) {
	int cnt_w = 0, cnt_l = 0;
